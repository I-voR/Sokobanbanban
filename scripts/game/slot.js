...
export const slots = {
    main: function() {
        let path = funcs.cwd() + escape('saves/')
        let saves = fs.readdirSync(path)
        let saveDefault = ',01,0,0,00-00-00.sav'
        let asc = escape('maps/ascending/01.map')
        
        switch (saves.length) {
        case 0:
            fs.copyFile(funcs.cwd() + asc, path + 1 + saveDefault, (err) => { if (err) throw err })
            fs.copyFile(funcs.cwd() + asc, path + 2 + saveDefault, (err) => { if (err) throw err })
            fs.copyFile(funcs.cwd() + asc, path + 3 + saveDefault, (err) => { if (err) throw err })
            break
        case 1:
